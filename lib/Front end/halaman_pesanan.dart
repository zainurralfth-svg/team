...
class _HalamanPesananState extends State<HalamanPesanan> {
                  ...
                  GestureDetector(
                    ...
                    child: Column(
                      children: [
                        ...
                        const Text('Kembali', style: TextStyle(color: Colors.white, fontSize: 10, fontWeight: FontWeight.bold)),
                      ],
                    ),
                  ),
                  const Expanded(
                    child: Padding(
                      ...
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text('Selamat Datang', style: TextStyle(color: Colors.white, fontSize: 22, fontFamily: 'Tai Heritage Pro', fontWeight: FontWeight.bold)),
                          Text('Dashboard Admin', style: TextStyle(color: Colors.white, fontSize: 22, fontFamily: 'Tai Heritage Pro', fontWeight: FontWeight.bold)),
                        ],
                      ),
                    ),
                  ),
                    ...
                    children: [
                      Row(
                        children: [
                          ...
                          const Text('Tambah pesanan', style: TextStyle(color: Colors.white, fontSize: 24, fontWeight: FontWeight.bold)),
                        ],
                      ),
                      ...
                      Center(
                        child: ElevatedButton.icon(
                          style: ElevatedButton.styleFrom(
                            backgroundColor: const Color(0xFFFFE8C4), foregroundColor: Colors.black,
  ...
  Widget _buildInputLabel(String text) {
    return Padding(padding: const EdgeInsets.only(bottom: 8.0, left: 5.0), child: Text(text, style: const TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold)));
  }
  ...
  Widget _buildTextField({bool isNumber = false}) {
    return TextField(
      ...
      decoration: InputDecoration(
        fillColor: const Color(0xFFFFE8C4), filled: true,
  ...
  Widget _buildDropdown() {
    return Container(
      ...
      decoration: BoxDecoration(color: const Color(0xFFFFE8C4), borderRadius: BorderRadius.circular(15)),
      child: DropdownButtonHideUnderline(
        child: DropdownButton<String>(
          isExpanded: true, value: _selectedMenu, hint: const Text('Pilih Menu', style: TextStyle(color: Colors.black54)),
          icon: const Icon(Icons.keyboard_arrow_down, color: Colors.black, size: 30),
          items: <String>['Taro Pudding', 'Brownie Burnt Cheesecake', 'Death By Chocolate', 'Mango Pudding'].map((String value) {
            return DropdownMenuItem<String>(value: value, child: Text(value));
          }).toList(),
          onChanged: (newValue) => setState(() => _selectedMenu = newValue),
        ),
      ),
    );
  }
